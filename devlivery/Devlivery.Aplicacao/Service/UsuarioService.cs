...
namespace Devlivery.Aplicacao.Service
{
    public class UsuarioService : IUsuarioService
    {
        ...
        public async Task<Resposta<string>> AtualizarPerfil(AlterarPerfilModel perfilAtualizado)
        {
            try
            {
                //var cadastro = _gerenciadorUsuario.CreateAsync(usuario, usuario.Senha);if (cadastro.IsCompleted)}
                var usuario = this.BuscarPorEmail(perfilAtualizado.Email);
                usuario.Nome = perfilAtualizado.Nome;
                usuario.Titulo = perfilAtualizado.Titulo;
                usuario.Senha = perfilAtualizado.Senha;
                usuario.Foto = perfilAtualizado.Foto;
                ...
                if (resposta > 0)
                {
                    return new Resposta<string>()
                    {
                        ...
                        Dados = new List<string>() { usuario.Email },
                        Status = 200,
                        Sucesso = true
                    };
                }
                else
                {
                    return new Resposta<string>()
                    {
                        Titulo = "Não foi possível atualizar o usuario",
                        Dados = new List<string>() { usuario.Email },
                        Status = 400,
                        Sucesso = false
                    };
            ...
            }catch(Exception ex)
            {
                ...
                {
                    Titulo = ex.Message,
                    Dados = new List<string>() { ex.Message, ex!.InnerException.ToString() },
                    Status = 400,
                    Sucesso = false
                };
            }
